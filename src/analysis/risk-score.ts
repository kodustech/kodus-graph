import type { GraphData, RiskScoreResult, BlastRadiusResult } from '../graph/types';
...
export function computeRiskScore(
  ...
  const changedFunctions = changedNodes.filter(
    n => n.kind === 'Function' || n.kind === 'Method',
  );
  const untestedCount = changedFunctions.filter(
    n => !testedFiles.has(n.file_path),
  ).length;
  const tgValue =
    changedFunctions.length > 0 ? untestedCount / changedFunctions.length : 0;

  // Factor 3: Complexity (0.20)
  const avgSize =
    changedNodes.length > 0
      ? changedNodes.reduce((s, n) => s + (n.line_end - n.line_start), 0) /
        changedNodes.length
      : 0;
  ...
  const hasInheritance = graph.edges.some(
    e =>
      ...
      changedSet.has(e.file_path),
  );
  const ihValue = hasInheritance ? 1 : 0;

  const score =
    brValue * 0.35 + tgValue * 0.3 + cxValue * 0.2 + ihValue * 0.15;
