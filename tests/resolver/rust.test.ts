...
import { resolve } from '../../src/languages/rust/resolver';
...
describe('Rust workspace path deps', () => {
    test('setup', () => {
        ...
        writeFileSync(join(TMP_WORKSPACE, 'Cargo.toml'), '[workspace]\nmembers = ["crates/*"]\n');
        writeFileSync(
            join(TMP_WORKSPACE, 'crates/app/Cargo.toml'),
            '[package]\nname = "app"\n\n[dependencies]\nshared = { path = "../shared" }\n',
        );
        writeFileSync(join(TMP_WORKSPACE, 'crates/shared/Cargo.toml'), '[package]\nname = "shared"\n');
    ...
    test('resolves shared::helper::format across workspace path dep', () => {
        const result = resolve(join(TMP_WORKSPACE, 'crates/app/src/main.rs'), 'shared::helper::format', TMP_WORKSPACE);
        expect(result).not.toBeNull();
        expect(result).toContain('helper.rs');
    });
...
describe('Rust mod patterns (file vs dir)', () => {
    test('setup', () => {
        ...
        writeFileSync(join(TMP_MOD_PATTERNS, 'Cargo.toml'), '[package]\nname = "mod-patterns"\nedition = "2021"\n');
    ...
    test('resolves crate::alpha to alpha.rs', () => {
        const result = resolve(join(TMP_MOD_PATTERNS, 'src/lib.rs'), 'crate::alpha', TMP_MOD_PATTERNS);
        expect(result).not.toBeNull();
        expect(result).toContain('alpha.rs');
    });

    test('resolves crate::beta to beta/mod.rs', () => {
        const result = resolve(join(TMP_MOD_PATTERNS, 'src/lib.rs'), 'crate::beta', TMP_MOD_PATTERNS);
        expect(result).not.toBeNull();
        expect(result).toContain('mod.rs');
    });
